  ...
  let content = {
    ...
    config((cfg, loc) => {
      ...
      // Show the slide number in the bottom right corner
      {
        set text(fill: ugent-blue, size: 17.1pt)
        let numbering = current-section
        let offset = (dx: (43.31cm / ratio - 1.4 * 2 * 2.56cm), dy: 24.86cm / ratio)
        let box_size = (width: 1.4 * 3 * 2.56cm / ratio, height: 1.44cm / ratio)

        place(top + left, ..offset, box(width: 9cm, align(horizon + right, numbering)))
      }
